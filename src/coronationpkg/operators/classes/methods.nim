...
proc methodbind(gdproc: GodotProc): Cloth = weave multiline:
  ...
  &"  let name = stringName \"{gdproc.native_name}\""
  &"  methodbind = interface_ClassDB_getMethodBind(addr className {gdproc.self.typesym}, addr name, {get gdproc.hash})"

proc weave(entry: ClassMethodPtrCallEntry): Cloth =
  let
    paramarray = "[" & entry.args.mapIt(&"getPtr {it.name}").join(", ") & "]"
    selfptr = case entry.self.isStatic
    of true: "nil"
    of false: &"CLASS_getOwner {entry.self.name}"
    paramptr =
...
proc weave(entry: ClassMethodVarargsVariantEntry): Cloth =
  let
    ...
    selfptr = case entry.self.isStatic
    of true: "nil"
    else: &"CLASS_getOwner {entry.self.name}"
    paramptr = case entry.args.len
    of 0: "nil"
